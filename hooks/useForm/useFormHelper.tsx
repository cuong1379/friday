export const isRequire = (error?: string) => (value: any, name: string) =>
  [null, undefined, NaN].includes(value) || value?.length === 0
    ? error || `${name} is required`
    : ''

export const isEmail = () => (value: string, name: string) => {
  return isRegex(/^[0-9a-zA-Z._-]+@[0-9a-zA-Z._-]+$/)(value, name)
}
...
export const isMin = (val: number, equal = false, error?: string) => (
  value: number,
  name: string
) =>
  (equal ? value >= val : value > val)
    ? ''
    : error || `${name} must greater ${equal ? 'or equal' : ''} than ${val}`

export const isMax = (val: number, equal = false, error?: string) => (
  value: number,
  name: string
) =>
  (equal ? value <= val : value < val)
    ? ''
    : error || `${name} must less ${equal ? 'or equal' : ''} than ${val}`

export const isMinLen = (val: number, equal = false, error?: string) => (
  value: string,
  name: string
) =>
...
export const isMaxLen = (val: number, equal = false, error?: string) => (
  value: string,
  name: string
) =>
  (equal ? value.length <= val : value.length < val)
...
export const phoneValidators = [isPhoneNumber(), isMinLen(8, false, `Phone's too short`)]
